...
<body>
    ...
    <div class="ord">
        <a href="order_report.php">Back</a>
        <h1>Order Report</h1>
    </div>
    ...
    <table id="example dataTables" class="table table-striped table-bordered" style="width:100%">
        ...
        if(isset($_GET['gene']))
        {
            ...
            if($to_date<$today)
            {
                $query = "SELECT * from order_tbl where Date between '$from_date' AND '$to_date'";
                ...
                if (mysqli_num_rows($run)) 
                {
                    ...
                    $results = mysqli_query($conn, "SELECT count(id) from order_tbl where Date between '$from_date' AND '$to_date'");
                    while ($row = mysqli_fetch_array($results)) 
                        {
                            echo '<h4>Total Order: '.$row['count(id)'].'</h4>';
                        }
                } 
            }
            else 
        ...
        else if(isset($_GET['td']))
        {
            ...
            if (mysqli_num_rows($run)) 
            {
                ...
                $results = mysqli_query($conn, "SELECT count(id) from order_tbl");
                while ($row = mysqli_fetch_array($results)) 
                    {
                        echo '<h4 style="margin-top:30px;">Total Orders: '.$row['count(id)'].'</h4>'; 
                    }
